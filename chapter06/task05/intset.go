...
// BIT_COUNT - количество разрядов.
const BIT_COUNT = 32 << (^uint(0) >> 63)

// Has указывает, содержит ли множество неотрицательное значение х.
func (s *IntSet) Has(x int) bool {
	word, bit := x/BIT_COUNT, uint(x%BIT_COUNT)
	...
	for i, word := range s.words {
		...
		for j := 0; j < BIT_COUNT; j++ {
			if word&(1<<uint(j)) != 0 {
				...
				fmt.Fprintf(&buf, "%d", BIT_COUNT*i+j)
			}
		}
	}
	buf.WriteByte('}')
...
func (s *IntSet) Copy() *IntSet {
	var cp IntSet
	if s.words != nil {
		cp.words = append(make([]uint, 0), s.words...)
	}
	return &cp
}
...
func (s *IntSet) Elems() (elems []int) {
	for i, word := range s.words {
		...
		for j := 0; j < BIT_COUNT; j++ {
			if word&(1<<uint(j)) != 0 {
				elems = append(elems, BIT_COUNT*i+j)
			}
		}
	}
	return
